...
def plot_result(feature, track_ce, train_acc, test_acc):
...
for feature in features:
    ...
    np.random.seed(6)
    # feature select
    dataset = ori_dataset.drop(columns=[feature])
    # train test split
    ...
    # build model
    Net = NeuralNetwork()
    Net.add_layer(NeuronLayer(input_num=5, output_num=8, activation='tanh',
                              input_layer=True))
    Net.add_layer(NeuronLayer(input_num=8, output_num=10, activation='tanh'))
    Net.add_layer(NeuronLayer(input_num=10, output_num=1,
                              activation='sigmoid'))
    # record
    track_ce = []
    train_acc = []
    ...
    for i in range(0, epoch_num):
        ...
        if ce_loss < best_train_loss:
            best_train_loss = ce_loss
    ...
    # display
    print()
    print(feature + 'best train loss' + str(best_train_loss))
    print(feature + 'best test loss' + str(best_test_loss))
    plot_result(feature, track_ce, train_acc, test_acc)
